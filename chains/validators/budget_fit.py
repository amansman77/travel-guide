...
def build_budget_fit_validator(llm: ChatOpenAI, parser: JsonOutputParser):
    """Build budget fit validator chain."""
    prompt = ChatPromptTemplate.from_messages([
        ("system", "You are a budget analyst for travel recommendations. Evaluate if the destination fits the traveler's budget constraints. Return ONLY valid JSON. No markdown."),
        ("user", """
Traveler profile:
{profile}
...
def run_budget_fit_validator(chain, profile: dict, candidate: dict, candidate_id: str) -> dict:
    """
    Execute budget fit validator.
